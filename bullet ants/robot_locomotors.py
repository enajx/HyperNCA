...
class WalkerBase(MJCFBasedRobot):
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    self.feet_contact = np.array([0.0 for f in self.foot_list], dtype=np.float64)
...
class AntAsymFront(WalkerBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerBase.__init__(self, "ant_asymfront.xml", "torso", action_dim=8, obs_dim=28, power=2.5)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.26 else -1  # 0.25 is central sphere rad, die if it scrapes the ground

class AntAsymCross(WalkerBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerBase.__init__(self, "ant_asymcross.xml", "torso", action_dim=8, obs_dim=28, power=2.5)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.26 else -1  # 0.25 is central sphere rad, die if it scrapes the ground

class AntBig(WalkerBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerBase.__init__(self, "ant_big.xml", "torso", action_dim=8, obs_dim=28, power=3)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.26 else -1  # 0.25 is central sphere rad, die if it scrapes the ground

class AntSmall(WalkerBase):
  foot_list = ['front_left_foot', 'front_right_foot', 'left_back_foot', 'right_back_foot']

  def __init__(self):
    WalkerBase.__init__(self, "ant_small.xml", "torso", action_dim=8, obs_dim=28, power=2)

  def alive_bonus(self, z, pitch):
    return +1 if z > 0.16 else -1  # 0.15 is central sphere rad, die if it scrapes the ground
